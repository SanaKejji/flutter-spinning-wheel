  ...
  SpinningWheel(
    ...
    this.shouldStartOrStop,
        ...
        assert(secondaryImage == null ||
            ((secondaryImageHeight ?? 0) <= height && (secondaryImageWidth ?? 0) <= width));
...
class _SpinningWheelState extends State<SpinningWheel> with SingleTickerProviderStateMixin {
  ...
  double get topSecondaryImage =>
      widget.secondaryImageTop ?? (widget.height / 2) - ((widget.secondaryImageHeight ?? 0) / 2);

  double get leftSecondaryImage =>
      widget.secondaryImageLeft ?? (widget.width / 2) - ((widget.secondaryImageWidth ?? 0) / 2);
            ...
            child: AnimatedBuilder(
                ...
                builder: (context, child) {
                  ...
                  return Transform.rotate(
                    angle: (_initialSpinAngle ?? 0) + _currentDistance,
                    child: child,
                  );
                }),
          ),
  ...
  void _moveWheel(DragUpdateDetails details) {
    ...
    if (_localPositionOnPanUpdate != null && _contains(_localPositionOnPanUpdate!)) {
      ...
      setState(() {
        ...
        _currentDistance = angle - (_initialSpinAngle ?? 0);
      });
    } else {
  ...
  void _startAnimation(Offset pixelsPerSecond) {
    var velocity =
        _spinVelocity.getVelocity((_localPositionOnPanUpdate ?? Offset(0, 0)), pixelsPerSecond);
